...
const ProductItem = ({ dataBetsSeller }) => {
    const { icons: { heart, reload, search }, image, title, star: { active, gray }, price, in_stock, href, point, saleOf } = dataBetsSeller
    ...
    const saleItem = checkSaleOf.includes(saleOf) ? (
    ...
    for (let i = 0; i < 5; i++) {
        stars.push(
            <img key={i} className="size-13 inline-block" src={i < point ? active : gray} alt="" />
        );
    }

    const starItem = stars
    return (
        <div>
            <li className="mt-6 md:mt-0 text-center group relative">
                <a href={href} className="bg-red">
                    {check_InStock}
                    {saleItem}
                ...
                <div className="mt-2 relative h-5 overflow-hidden">
                    ...
                    <div
                        className="absolute left-1/2 -translate-x-1/2 group-hover:bottom-0 -bottom-5 transition-all duration-300">
